...
class SocialAccountTests(TestCase):
    ...
    def test_email_address_clash_username_required(self):
        ...
        # POST different username/email to socialss signup form
        request.method = "POST"
    ...
    def test_email_address_clash_username_not_required(self):
        ...
        # POST email to socialss signup form (username not present)
        request.method = "POST"
    ...
    def _email_address_clash(self, username, email):
        ...
        # A socialss user being signed up...
        account = SocialAccount(provider="twitter", uid="123")
        user = User()
        ...
        # Signing up, should pop up the socialss signup form
        factory = RequestFactory()
        request = factory.get("/accounts/twitter/login/callback/")
        request.user = AnonymousUser()
        SessionMiddleware(lambda request: None).process_request(request)
    ...
    def test_verified_email_change_at_signup(self):
        """
        Test scenario for when the user changes email at socialss signup. Current
    ...
    def test_unverified_email_change_at_signup(self):
        """
        Test scenario for when the user changes email at socialss signup, while
    ...
    def test_social_account_taken_at_signup(self):
        """
        Test scenario for when the user signs up with a socialss account
        and uses email address in that socialss account. But upon seeing the
        ...
        the same socialss account token which is owned by their first attempt.
        """
        session = self.client.session
        User = get_user_model()
        sociallogin = SocialLogin(
